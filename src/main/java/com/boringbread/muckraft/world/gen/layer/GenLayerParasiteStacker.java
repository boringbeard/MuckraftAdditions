...
public class GenLayerParasiteStacker extends GenLayer
{
    ...
    public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight)
    {
        ...
        for (int y = 0; y < 8; y++)
        {
            ...
            noise = perlin.generateNoiseOctaves(noise, areaX, areaY, areaWidth, areaHeight, 0.25, 0.25, 0);

            for (int x = 0; x < areaWidth; x++)
            {
                for (int z = 0; z < areaHeight; z++)
                {
                    int previousHeight = (int) MathHelper.clamp(previousNoise[x * areaHeight + z] * 16, -16, 16);
                    int currentHeight = y == 7 ? 32 : (int) MathHelper.clamp(noise[x * areaHeight + z] * 16, -16, 16) + 32;
